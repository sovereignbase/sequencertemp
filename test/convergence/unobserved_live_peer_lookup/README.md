...
After four complete editing rounds both peers report eight visible Frames. The
first projection read then checks every index and must return:
...
structural tail at `find(7)`. Snapshot reconstruction is not involved; the
