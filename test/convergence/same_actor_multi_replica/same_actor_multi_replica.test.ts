...
const expectConverged = <T>(
  ...
  for (const replica of replicas) {
    expect(replica.visibleFrameCount).toBe(expected.visibleFrameCount)
...
describe('same actor in multiple replicas', () => {
  it('keeps per-instance Clocks independent across concurrent tabs', () => {
    ...
    const leftReplace = left.replace(['left-r0', 'left-r1'], 2, 4)
    const rightInsert = right.insert(['right-0', 'right-1'], 3)
    const rightRemove = right.remove(1, 3)
    ...
    const leftTail = left.insert(['left-tail'], left.visibleFrameCount)
    const rightHead = right.replace(['right-head'], 0, 1)
